...
from preparation.editor2.models.st_file_tree_item import STFileTreeItem
...
class STFileTreeModel(QAbstractItemModel):
    ...
    def index(self, row, column, parent=QModelIndex()):
        """Создает и возвращает индекс для элемента модели.

        Индекс связывает конкретную позицию (строка/столбец) с данными в модели, позволяя View-компонентам
        запрашивать информацию для отображения. Родительский индекс определяет контекст иерархии.

        Args:
            row (int): Номер строки элемента (0-based).
            column (int): Номер столбца элемента (0-based). Для плоских списков обычно 0.
            parent (QModelIndex, optional): Индекс родительского элемента. Если невалиден (по умолчанию),
                элемент считается корневым.

        Returns:
            QModelIndex: Валидный индекс с привязкой к данным, если позиция существует,
            иначе невалидный QModelIndex().

        Raises:
            RuntimeError: Если row/column выходят за границы допустимого (неявно, через hasIndex()).

        Notes:
            - Для работы требует переопределения hasIndex() и реализации структуры данных (например,
              self.root_item и child_items в примере).
            - internalPointer() возвращает указатель на данные (void* в C++, здесь - Python-объект).
            - createIndex() связывает индекс с данными (child_item), которые можно извлечь позже
              через internalPointer().
        """
