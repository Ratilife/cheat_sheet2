...
//Эта строка объявляет, что данный файл описывает лексическую грамматику (лексер) и называет её CSharpLexer. 
//Лексер — это первая фаза синтаксического анализа, которая разбивает входной поток текста на отдельные токены.

channels {
    COMMENTS_CHANNEL,
    DIRECTIVE
}
//Здесь определяются каналы.  Каналы позволяют группировать токены по типу. В данном случае:

    //COMMENTS_CHANNEL: Токены комментариев будут направлены на этот канал. Это позволяет отфильтровать комментарии при построении абстрактного синтаксического дерева (AST).
    //DIRECTIVE: Токены препроцессора (директив) будут направлены сюда. Аналогично комментариям, они могут быть отфильтрованы при парсинге.

options {
    superClass = CSharpLexerBase;
}
//Эта опция указывает, что генерируемый лексер должен наследовать от класса CSharpLexerBase.  Это может быть полезно для расширения функциональности лексера.

BYTE_ORDER_MARK: '\u00EF\u00BB\u00BF';
//Это правило определяет байт-порядок-маркер (BOM) для UTF-8 кодировки.  ANTLR будет игнорировать его.
